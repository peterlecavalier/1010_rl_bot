...
class game_1010_v0(Env):
    ...
    def reset(self):
        ...
        return {"action_mask": self.valid_moves, "observations": (self.grid, self.pieces)}
    ...
    def step(self, action):
        ...
        return {"action_mask": self.valid_moves, "observations": (self.grid, self.pieces)}, reward, done, {}
    ...
    def check_valid_move(self, action, ret_grid=True, bad_reward=0):
        ''' 
        Returns:
...
class game_1010_v1(Env):
    def __init__(self, seed=None):
        super(game_1010_v1, self).__init__()

        # Grid observation space
        self.grid_shape = (10, 10)
        self.obs_space = spaces.Box(low=0, high=20, shape=(103,), dtype=int)
        self.valid_mask_space = spaces.Box(low=0, high=1, shape=(300,), dtype=int)
    ...
    def step(self, action):
        ...
        return {"action_mask": self.valid_moves, "observations": np.concatenate((self.grid.flatten(), self.pieces))}, reward, done, {}
    ...
    def check_valid_move(self, action, ret_grid=True, bad_reward=-50000):
        ''' 
        Returns:
            done: Boolean - whether the action ends the game or not
            grid (optional): ndarray - returns the grid with piece added
...
if __name__ == "__main__":   
    ...
    while True:
        # Take a random action
        for i in range(300):
            done = env.check_valid_move(i, ret_grid=False)
            print(f"Action {i}: {not done}")
